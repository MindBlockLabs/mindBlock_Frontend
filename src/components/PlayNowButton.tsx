import styled from "styled-components";
import { useNavigate } from "react-router-dom";

const Button = () => {
  const navigate = useNavigate();

  return (
    <StyledWrapper>
      <button className="button" onClick={() => navigate('/auth')}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <g strokeWidth={0} id="SVGRepo_bgCarrier" />
          <g
            strokeLinejoin="round"
            strokeLinecap="round"
            id="SVGRepo_tracerCarrier"
          />
          <g id="SVGRepo_iconCarrier">
            {" "}
            <path
              fill="#ffffff"
              d="M12.75 6V3.75H11.25V6L9 6C6.10051 6 3.75 8.3505 3.75 11.25V17.909C3.75 19.2019 4.7981 20.25 6.09099 20.25C6.71186 20.25 7.3073 20.0034 7.74632 19.5643L10.8107 16.5H13.1893L16.2537 19.5643C16.6927 20.0034 17.2881 20.25 17.909 20.25C19.2019 20.25 20.25 19.2019 20.25 17.909V11.25C20.25 8.3505 17.8995 6 15 6L12.75 6ZM18.75 11.25C18.75 9.17893 17.0711 7.5 15 7.5L9 7.5C6.92893 7.5 5.25 9.17893 5.25 11.25V17.909C5.25 18.3735 5.62652 18.75 6.09099 18.75C6.31403 18.75 6.52794 18.6614 6.68566 18.5037L10.1893 15H13.8107L17.3143 18.5037C17.4721 18.6614 17.686 18.75 17.909 18.75C18.3735 18.75 18.75 18.3735 18.75 17.909V11.25ZM6.75 12.75V11.25H8.25V9.75H9.75V11.25H11.25V12.75H9.75V14.25H8.25V12.75H6.75ZM15 10.875C15 11.4963 14.4963 12 13.875 12C13.2537 12 12.75 11.4963 12.75 10.875C12.75 10.2537 13.2537 9.75 13.875 9.75C14.4963 9.75 15 10.2537 15 10.875ZM16.125 14.25C16.7463 14.25 17.25 13.7463 17.25 13.125C17.25 12.5037 16.7463 12 16.125 12C15.5037 12 15 12.5037 15 13.125C15 13.7463 15.5037 14.25 16.125 14.25Z"
              clipRule="evenodd"
              fillRule="evenodd"
            />{" "}
          </g>
        </svg>
        Play Now
      </button>
    </StyledWrapper>
  );
};

const StyledWrapper = styled.div`
  /* Add display style if needed */
  display: inline-block;

  .button {
    display: flex;
    justify-content: center;
    align-items: center;
    border-radius: 7px;
    border: none;
    /* Updated gradient to use cyan/pink theme */
    background: linear-gradient(to right, #00e5ff, #ff0080);
    color: white;
    font-family: inherit;
    text-align: center;
    font-size: 13px;
    /* Adjusted shadow to match theme */
    box-shadow: 0px 14px 56px -11px #00e5ffaa; /* Cyan shadow with some transparency */
    width: 11em;
    padding: 0.7em;
    transition: all 0.4s;
    cursor: pointer;
    height: 3rem; /* Match height of LearnMoreButton */
  }

  .button svg {
    width: 25px;
    height: 25px;
    margin-right: 5px; /* Add some space between icon and text */
  }

  .button svg path {
    fill: white; /* Ensure icon fill is white */
  }

  .button span {
    cursor: pointer;
    display: inline-block;
    position: relative;
    transition: 0.4s;
  }

  .button span:after {
    content: "for free";
    position: absolute;
    opacity: 0;
    top: 0;
    right: -20px;
    transition: 0.7s;
  }

  .button:hover span {
    padding-right: 3.8em;
  }

  .button:hover span:after {
    opacity: 1; /* Changed from 4 to 1 for standard opacity */
    right: 0;
  }

  /* Add hover effect for background */
  .button:hover {
    background: linear-gradient(
      to right,
      #00cfff,
      #f00070 /* Slightly darker/adjusted hover colors */
    );
    box-shadow: 0px 14px 56px -11px #ff0080aa; /* Pink shadow on hover */
  }
`;

export default Button;
